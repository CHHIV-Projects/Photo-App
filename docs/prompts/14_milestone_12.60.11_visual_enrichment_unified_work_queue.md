...
# Answers to Coder Questions — Milestone 12.60.11

## 1. Completed card removal / reject-ignore persistence

Use a hybrid approach:

```text
If a matching google_vision landmark observation exists:
  persist Reject/Ignore to place_observations.status

If no matching observation exists:
  remove from active queue in session state only

Reason:

Reject/Ignore should be durable when there is provider evidence to review.
But if the card has no observation yet, there is nothing meaningful to persist.

Behavior:

Reject Suggestions:
  mark current pending/suggestion observation(s) rejected where possible
  remove card from active queue

Ignore Asset:
  mark relevant pending/suggestion observation(s) ignored where possible
  remove card from active queue

No observation exists:
  remove card from current queue only
  do not create a synthetic rejected/ignored observation

Do not overbuild a new review-state table in this milestone.

2. Canonical normalization fallback

If the canonical asset is missing or not visible, fall back to the selected visible asset from the same duplicate group.

Preferred behavior:

If canonical exists and is visible:
  use canonical representative

If canonical is missing / unavailable / non-visible:
  use selected visible asset as representative
  show small warning/note:
    Canonical representative unavailable; using selected asset.

Do not skip the group entirely unless no visible usable asset exists.

Reason:

The user selected an asset intentionally.
The workflow should not drop it just because canonical metadata is imperfect.
3. Previously scanned definition

For 12.60.11, “Previously scanned” should be based on any prior Google Vision landmark observation for that asset, regardless of status:
...
Previously scanned = asset has any source_type=google_vision and observation_type=landmark observation.

Do not require successful/non-empty diagnostics for this label in 12.60.11.

Reason:

The main user question is: has this asset already been sent through the landmark workflow?

If later we need more detail, we can distinguish:

Previously scanned — no landmark found
Previously scanned — suggestions available
Previously reviewed

But for now, keep it simple.

4. Force Rescan behavior

Force Rescan should rerun Landmark Detection only by default.
...
Force Rescan should mean rerun the primary landmark pass.
Run More Context already exists for Web/Label/Object diagnostics.

If coder wants to preserve last-used feature mix, defer that. It adds unnecessary state complexity.

5. Apply-to-duplicate-group checkbox

Yes. Show Apply to duplicate group only when:

asset has duplicate_group_id
and
there are eligible duplicate-group members beyond the representative

Default:

checked

User should be able to uncheck.
...
The workflow is canonical-first, so applying accepted context to duplicate-group members is usually desired.
But it should still be visible and controllable, not silent.
6. Propagation partial failure

If primary accept succeeds but duplicate propagation partially fails, the card should still leave the active queue.

Show warning/toast/banner:

Context accepted.
Duplicate-group propagation partially failed:
Added: X
Already present: Y
Failed: Z
...
The main decision for the representative asset succeeded.
Propagation is secondary.

If the primary context label creation fails, do not remove the card.

7. Empty queue state

Yes. When selected queue clears, hide collection/legacy tools behind one collapsed Advanced/Legacy disclosure by default.
...
Below that, collapsed:

Advanced / Legacy Candidate Sources

Do not dump the user back into the old Candidate Selection / Landmark Candidates screen.

8. Show-only suggestions / unresolved filter

Include this only if trivial.

Priority:

Primary requirement:
  unified queue behavior

Secondary if easy:
  Show All / Suggestions Available / Unresolved

If it adds complexity, defer to 12.60.12.

Minimum for 12.60.11:

Cards leave the queue after action.
Clear Queue exists.
Previously scanned state exists.
Implementation Direction Confirmation

Proceed with coder’s recommended implementation shape:

- Introduce explicit queue item state in VisualEnrichmentView.
- Normalize selected working set upfront by duplicate_group_id and canonical representative.
- Show collapse summary banner when selected assets are reduced to canonical work items.
- Replace “Candidate Selection” heading with “Visual Enrichment Work Queue.”
- Hide collection/legacy queue behind Advanced/Legacy disclosure.
- Persist accept/manual as today.
- Persist reject/ignore to matching observations when they exist; otherwise session-only queue removal.
- Add Previously scanned status.
- Add Force Rescan per card, landmark-only by default.
- Add Apply to duplicate group checkbox where eligible, default checked.
- Use existing propagation endpoint after primary context label creation.
Safety boundaries
...
- No silent duplicate propagation without visible checkbox.
- No duplicate group/canonical mutation.
- No automatic Vision run on page load.
Priority order

If scope pressure occurs, prioritize:

1. Unified work queue.
2. Hide legacy/candidate sections.
3. Canonical/duplicate-group normalization.
4. Card removal after decision.
5. Clear Queue clean empty state.
6. Previously scanned + Force Rescan.
7. Apply-to-duplicate-group default checked.
8. Show-only suggestions/unresolved filter.