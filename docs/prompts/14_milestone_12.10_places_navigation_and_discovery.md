...
Use these defaults for 12.10.

## Confirmed decisions for 12.10

1. Representative thumbnail for a place
- Use the **most recent asset** in the place as the representative thumbnail
- Determine this by:
  - `captured_at DESC`
  - then stable deterministic tie-breakers if needed

Reason:
- gives a more useful and current visual face for the place
- still deterministic
- better browsing UX than first-assigned for this milestone

---

2. Shared photo grid vs refactor scope
- Use **Option B**
- Improve `PlacesView.tsx`’s own photo grid so it looks and behaves consistently with Photos
- Do **not** extract a new shared `PhotoGrid` component in 12.10

Reason:
- lower risk
- avoids unnecessary refactor in this milestone
- keeps scope disciplined while still improving consistency

---

3. Existing coordinate filter in Places list
- **Keep it as-is** in 12.10
- Do not remove it just because broader Places search is out of scope

Reason:
- it already exists
- removing working functionality would be unnecessary churn
- 12.10 is about improving Places usability, not reducing existing capability

Clarification:
- do not expand it into a broader Places search feature in this milestone
- just preserve the existing filter if already present

---

4. Photo detail panel behavior from Places
- **Yes, 12.10 should explicitly ensure this works correctly**
- Clicking a photo in Places must reliably open the existing photo detail experience

Even if it partially works today, treat this as an explicit 12.10 requirement to verify and fix if needed.

Reason:
- this is central to Places being a first-class browsing surface
- users must be able to move from place browsing into normal photo inspection cleanly

---

## Summary of intended 12.10 behavior

- place thumbnail = most recent asset in the place
- improve PlacesView grid directly (no shared-component refactor)
- keep existing coordinate filter
- explicitly verify/fix photo detail opening from Places
