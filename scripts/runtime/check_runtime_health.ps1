...
Write-Host "  Photo Organizer Health Check`n" -ForegroundColor Blue
...
if ($backendUp) {
    # Try health endpoint
    $healthOk = Test-HttpEndpoint "http://127.0.0.1:$BackendPort/health"
    Write-HealthResult "Backend" $healthOk "localhost:$BackendPort (/health)"
    if (-not $healthOk) { $allHealthy = $false }
} else {
...
if ($allHealthy) {
    Write-Host "??? All critical services are healthy`n" -ForegroundColor Green
    exit 0
} else {
    Write-Host "??? Some services are not healthy. Please check above.`n" -ForegroundColor Red
