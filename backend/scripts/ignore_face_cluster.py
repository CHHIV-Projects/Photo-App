"""Mark a face cluster as ignored for labeling workflows."""
...
USAGE = "Usage: python ignore_face_cluster.py [--no-prompt] <cluster_id>"
...
def _parse_cluster_id(argv: list[str]) -> int:
    ...
    if args:
        ...
        try:
            raw = input("Enter cluster ID to ignore: ").strip()
...
def main() -> int:
    ...
    db = SessionLocal()
    try:
        result = set_cluster_ignored(db, cluster_id, True)
    except ValueError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    finally:
