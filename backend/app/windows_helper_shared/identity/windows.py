"""Windows non-admin read-only identity evidence collector."""
...
def _classify_boundary(source_type: SourceType, observed_path: str | None) -> FilesystemBoundaryType:
...
def _has_evidence_code(
    evidence: list[NormalizedIdentityEvidence],
    *,
    category: str,
    code: str,
) -> bool:
...
class WindowsIdentityCollector:
    """Read-only observational collector based on non-admin Windows evidence."""
    ...
    def capabilities(self) -> IdentityCollectorCapabilities:
        return IdentityCollectorCapabilities(
    ...
    def collect(self, request: IdentityCollectionRequest) -> IdentityCollectionResult:
        if request.source_type == "cloud":
            return self._cloud_response(request)

        evidence: list[NormalizedIdentityEvidence] = []
        warnings: list[NormalizedIdentityEvidence] = []
        blockers: list[NormalizedIdentityEvidence] = []
    ...
    def _cloud_response(self, request: IdentityCollectionRequest) -> IdentityCollectionResult:
    ...
    ) -> tuple[list[NormalizedIdentityEvidence], list[NormalizedIdentityEvidence]]:
        commands = [
            ["cmd", "/c", "vol", drive],
    ...
    ) -> tuple[list[NormalizedIdentityEvidence], list[NormalizedIdentityEvidence], list[NormalizedIdentityEvidence]]:
        evidence: list[NormalizedIdentityEvidence] = []
        warnings: list[NormalizedIdentityEvidence] = []
        blockers: list[NormalizedIdentityEvidence] = []
    ...
    def _collect_optical_metadata(
        self,
        drive: str,
    ) -> tuple[_OpticalMediaMetadata, list[NormalizedIdentityEvidence], list[NormalizedIdentityEvidence]]:
        drive_letter = drive.rstrip(":\\/")
        result = self._command_runner.run(
    ...
    def _optical_unreadable_blocker(self, metadata: _OpticalMediaMetadata) -> NormalizedIdentityEvidence:
    ...
    def _movie_disc_blocker(self, root_names: tuple[str, ...]) -> NormalizedIdentityEvidence | None:
    ...
    def _storage_metadata_blockers(
        ...
        *,
        ...
        path_status: PathProbeStatus,
        evidence: list[NormalizedIdentityEvidence],
    ) -> list[NormalizedIdentityEvidence]:
        blockers: list[NormalizedIdentityEvidence] = []
    ...
    def _collect_network_mapping_evidence(self) -> tuple[list[NormalizedIdentityEvidence], list[NormalizedIdentityEvidence]]:
    ...
    def _build_root_candidate(
        ...
        if source_type == "cloud":
            return ProviderNativeRootEvidence(
    ...
    def _classify_safety(
        ...
        *,
        ...
        if request.comparison_requested:
