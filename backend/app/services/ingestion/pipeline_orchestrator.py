"""Callable pipeline orchestration for source intake and ingestion.

This module contains the core pipeline logic extracted from scripts/run_pipeline.py
so that both the CLI wrapper and the Admin background service can reuse the same
ingestion behavior without duplication.

CLI usage: scripts/run_pipeline.py imports from here and provides argument parsing.
Admin usage: app/services/admin/source_intake_execution_service.py calls _run_pipeline()
             with stop_requested_fn and on_batch_complete callbacks.
"""
...
# backend/ folder (app/services/ingestion/ → parents[3])
...
def _face_detection_stage(ctx: PipelineContext) -> dict[str, Any]:
    ...
    if not Path(model_path).is_absolute():
        model_path = str(resolve_runtime_path(model_path))
...
def _skip_stage(
    *, key: str, index: int, total: int, label: str, reason: str, outcomes: list[StageOutcome]
) -> None:
...
def _run_batch_loop(
    ...
    *,
    ...
    on_batch_complete: Callable[[PipelineContext], None] | None = None,
) -> None:
    ...
    stop_requested_fn: called before each batch; if returns True, stops gracefully.
    on_batch_complete: called after each batch completes (used for progress reporting).
    """
    while ctx.processing_records:
        ...
        if on_batch_complete is not None:
            try:
                on_batch_complete(ctx)
            except Exception:  # noqa: BLE001
                pass  # progress callback errors must not break the pipeline
        if ctx.from_path is not None:
