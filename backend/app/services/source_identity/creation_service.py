...
class SourceCreationService:
    ...
    def _build_plan(
        ...
        if probe is not None and probe.os_family == "linux":
            legacy_linux_sources = [source for source in exact_sources if _is_legacy_root_model(source)]
            if legacy_linux_sources:
                blockers.append(
                    _message(
                        "mounted_legacy_source_conflict",
                        "An existing legacy Source uses this Mounted root and cannot be relinked automatically.",
                    )
                )
                exact_sources = [source for source in exact_sources if not _is_legacy_root_model(source)]
        reference_summaries = {
