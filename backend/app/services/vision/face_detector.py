...
# Extensions where cv2.imread may silently load raw unrotated sensor data
# (ignoring EXIF/HEIF orientation).  Pillow+pillow-heif always decodes these
# with orientation applied, matching the JPEG preview generated by
# preview_service.py, so face bbox coordinates stay consistent.
_PILLOW_ONLY_EXTENSIONS = frozenset({".heic", ".heif"})


def load_image_for_cv2(path: str | Path):
    """Load an image as a BGR NumPy array compatible with OpenCV.

    For HEIC/HEIF files Pillow is always used (skipping cv2) so that face
    bbox coordinates are in the same correctly-oriented pixel space as the
    JPEG preview generated by preview_service.py.  For all other formats
    cv2.imread is tried first (fast path) with Pillow as a fallback.

    Returns None only when both methods fail (unreadable file / corrupt data).
    """
    ext = Path(path).suffix.lower()
    if ext not in _PILLOW_ONLY_EXTENSIONS:
        image = cv2.imread(str(path))
        if image is not None:
            return image

    # Pillow path: always used for HEIC/HEIF; fallback for everything else.
    try:
        with Image.open(path) as pil_img:
            rgb = pil_img.convert("RGB")
            bgr = np.array(rgb)[:, :, ::-1]  # RGB → BGR
            return bgr
