...
class LinuxSourceIdentityBrokerTests(unittest.TestCase):
    ...
    def test_windows_style_relative_root_is_rejected(self) -> None:
        with self.assertRaises(broker_module.BrokerError) as error:
            self._broker().handle(
                {
                    "protocol_version": 1,
                    "action": "probe",
        ...
        self.assertEqual(error.exception.code, "relative_root_traversal")
